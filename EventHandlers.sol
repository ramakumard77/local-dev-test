...
contract EventHandlers {
    ...
    event Launch(address indexed to, address indexed from);
...
contract DAOLaunch is ERC20 {
    ...
    event Mint(address indexed from, uint256 indexed tokents);
    event Burn(address indexed from, uint256 indexed tokents);
    ...
    function burnToken(uint256 burnCount) public onlyOwner {
        require( burnCount < maxSupply , "Minimum Token Supply required to burn");
        require(owner != address(0), "Owner cannot be 0 address");
        
        _burn(owner, burnCount);
        emit Burn(msg.sender, burnCount);
